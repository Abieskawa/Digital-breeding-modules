...
- `DGbreeding-biotools`: alignment/variant calling/QC (fastp, bwa-mem2, samtools, bcftools, glnexus, plink, etc.)
...
Core steps (`step=1-5` in config):
    ...
    01_DNAseq_alignment/*.bam + *.bai
        ...
        |     plink QC -> cohort.filtered.vcf.gz
        |     plink LD-prune (optional) -> cohort.filtered.ldpruned.vcf.gz
        v
    02_Variant_Calling/
        |
...
Evaluation steps (`eva_step=0-5` in config, can be run alongside or after core steps):
...
- `5`: ROC/AUC plots -> `<prediction_output_dir>/AUC_dir/auc_comparison_*.png`, `auc_summary.csv`
...
| PLINK | https://www.cog-genomics.org/plink/ | Purcell et al., 2007, AJHG; Chang et al., 2015, GigaScience |
